...
import { Card, Row, Col, List, Tag, Divider, Typography, Button } from 'antd';
...
const PipelineView = ({ pipeline, onClose, hideHeader = false }) => {
    ...
    // Sort each type's stages by order
    ...
    // Set default selected type to first type if available
    const [selectedType, setSelectedType] = useState(stageTypes.length > 0 ? stageTypes[0] : null);

    // Update selectedType if stageTypes changes and current selection is not valid
    useEffect(() => {
    ...
    // Handle card click to filter stages
    const handleCardClick = (type) => {
        setSelectedType(type);
    };

    // Filter stage types based on selection
